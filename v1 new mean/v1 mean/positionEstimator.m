...
% for i = 1:98
%     numspikes = sum(test_data.spikes(i,t_0:t_1));
...
elseif length(test_data.spikes)>length(modelParameters(1).v_predict(direction).meanX)
    x= modelParameters(1).v_predict(direction).meanX(end);
    y= modelParameters(1).v_predict(direction).meanY(end);
    "elseif"
else
    % s = s_0 + v*t
...
%     y = test_data.decodedHandPos(2,length(test_data.decodedHandPos(2,:))) + velocity_y / fs * timegap;
    x= modelParameters(1).v_predict(direction).meanX(length(test_data.spikes)-320);
    y= modelParameters(1).v_predict(direction).meanY(length(test_data.spikes)-320);
    "else"
end
